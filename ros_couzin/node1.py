...
import ros
# 信息列表
from topic_demo.msg import agent
from topic_demo.msg import agents
...
class Couzin():
    ...
    def talker(self):
        ...
        for i in range(len(self.swarm)):
            agents.append(agent(self.swarm[i].id,0,0,
                                self.swarm[i].vel[0],self.swarm[i].vel[0]))
        while not rospy.is_shutdown():
            # 发布
            rospy.loginfo("node1:agents_infor:%s","\n".join([f"id: {person.id}, x: {person.pos[0]}, y: {person.pos[1]}, v_x:{person.vel[0]}, v_x:{person.vel[1]}" for person in agents]))
            pub.publish(agents)
            rate.sleep()
    ...
    def listerner(self):
        ...
        def callback(agents):
            ...
            for i in range(len(agents)):
                for j in range(len(self.swarm)):
                    if agents[i].id == self.swarm[j].id:
                        self.swarm[j].pos[0] = agent[i].x
                        self.swarm[j].pos[1] = agent[i].y
