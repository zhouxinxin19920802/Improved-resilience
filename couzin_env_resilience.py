...
class Couzin():
    ...
    def step(self, actions, failure_list):
            ...
            if math.sqrt(
                    ...
                    pow(agent.pos[0] - self.target_x, 2) + pow(agent.pos[1] - self.target_y, 2)) > self.target_radius:
                        ...
                        if cal_angle_of_vector(r_normalized, agent_vel_normalized) < agent.field_of_view / 2:
                            if norm_r < self.a_minimal_range:
                                ...
                                # 加一个判断，如果在异常个体列表中，则不加入观察域
                                if(neighbor.id not in failure_list):
                                    # 添加吸引区域邻域集合
                ...
                if norm(dr) != 0:
                    ...
                    if agent.is_leader:
                        ...
                        # logging.info("wp:{},{}".format(agent.w_p,agent.field_of_view))
        ...
        if len(failure_list) !=0 and self.total_steps >= 200:
            ...
            if time_flag == True and self.write_once_flag == True:
                with open("time_flag_step_xu1.txt","w") as flag_step:
        ...
        if self.is_visual:
            ...
            for k in range(len(self.swarm)):
                if(self.swarm[k].id in failure_list):
                    continue
        ...
        if not evaluate:
            ...
            if self.total_steps >= 2000:
                ...
                with open("destination_nums.txt","a+") as space:
                    space.write(str(count_fn)+"\n") 

                with open("destination_nums1.txt","a+") as space:
                    space.write(str(count_fn_leader)+"\n") 
        ...
        if(all(done)==True and self.space_time_flag == True) or (self.space_time_flag == True and count_fn > 0):
            ...
            # 写入空间复杂度和时间复杂度
            with open("space_complexity_r1.txt","a+") as space:
                space.write(str(space_complexity)+"\n") 
            with open("time_complexity_r1.txt","a+") as space:
        ...
        with open("performance_curve_xu1.txt","a+") as performance_curve:
    ...
    # 在计算连通度时添加一个功能，给包含领导者的最大连通子图赋值。
    def connectivity_cal(self):
        connectivity = 0

        # 计算连通度
        class Uav:
            def __init__(self, id, p_x, p_y,pos):
                self.id = id
                self.p_x = p_x
                self.p_y = p_y
                self.pos = pos
        ...
        for item in self.swarm:
            uavs.append(Uav(item.id, item.pos[0], item.pos[1],item.pos))
        # 计算分群
...
def resilience_cal_display():
    ...
    # 获取性能数据，
    data = []
    with open("performance_curve_xu1.txt", "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip("\n")
            data.append(float(line))
    times_stable = 0
    with open("time_flag_step_xu1.txt", "r", encoding="utf-8") as f:
        for line in f:
    ...
    # 删除
    try:
        delete_file_if_exists("performance_curve_xu1.txt")
        delete_file_if_exists("time_flag_step_xu1.txt")
    except OSError as e:
        print(f"删除文件时发生错误: {e}")
