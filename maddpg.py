...
class MADDPG:
    def __init__(self, actor_dims, critic_dims, n_agents, n_actions, env1,
                 alpha=1e-4, beta=1e-3, fc1=64, fc2=64, gamma=0.95, tau=0.01,
                 chkpt_dir='tmp/maddpg/', scenario='co-op_navigation'):
        self.agents = []
        chkpt_dir += scenario
        for agent_idx in range(n_agents):
            agent = list(env1.action_spaces.keys())[agent_idx]
            min_action = env1.action_space(agent).low
            max_action = env1.action_space(agent).high
    ...
    def choose_action(self, raw_obs, evaluate=False):
        actions = {}
        logging.info("raw_obs:{}".format(raw_obs))
        for agent_id, agent in zip(raw_obs, self.agents):
            logging.info("id:{}".format(agent_id))
