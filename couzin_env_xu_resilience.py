...
if __name__ == '__main__':
    ...
    for ngame in range(100):
        ...
        for i in range(2000):
            ...
            obs1_,  reward_temp, done = couzin.step(actions=actions,failure_list = failure_temp)
            list_reward = list(reward_temp)
            if True in done:
                break
            score += sum(list_reward)
        resilience_cal_display()
        print("ngame:",ngame)

        
    # 计算resilience的平均值
    cons = data_average("resilience_xu.txt")
    print("cons:",cons)
